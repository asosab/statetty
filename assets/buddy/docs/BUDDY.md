...
La carga es asíncrona y secuencial por dependencias: `config.js` → personaje (`modules/character`) → assets del personaje → resto de módulos. El orden de la carga DE los módulos lo impone `MODULE_LOAD_ORDER` en `buddy.js` (dependencias reales: `telemetry` antes de `wa_listener` y de quien publique eventos; `says` antes de `archeryGame`/`archerySchool` que usan `window.buddy_says`), NO el orden alfabético que pueda devolver la config de BD (donde `order` suele valer 100 para todos). `getConfiguredModules()` reordena de forma estable según esa secuencia y coloca al final los módulos no listados. Buddy expone un evento `buddy:ready` y una API pública (`window.Buddy`) con las capacidades activas (`window.Buddy.modules.isActive('x')`), resolución de assets con herencia personaje→módulo, y una política común de "ocupado" (visibilidad de pestaña + providers registrados por cada módulo) para no interrumpir al usuario con mensajes fuera de momento.
